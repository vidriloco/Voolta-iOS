...
- (void) buildContentForElement:(BrochureElement *)element onContainer:(UIView *)container
{
    if ([element isParagraph]) {
        ...
        // TODO: Fix legend view
        float hInset = -2;
        if ([element requiresLeftAligment]) {
            hInset = 17;
        }
        [self drawLegendElementOnContainer:container withBrochureElement:element withHorizontalInset:hInset];
...
- (void) drawParagraphOnContainer:(UIView*)container
    ...
    [ContentBuilder setText:element.paragraphContent withNSAlignment:NSTextAlignmentJustified onLabel:blockIntroTextView andFontSize:15];
...
- (void) drawLegendElementOnContainer:(UIView*)container
                  withBrochureElement:(BrochureElement *)element
                  withHorizontalInset:(float)horizontalInset {
    ...
    [legendElementView setFrame:CGRectMake(horizontalInset, _lastOffset.y, legendElementView.frame.size.width, legendElementView.frame.size.height)];
...
+ (void) setText:(NSString *)text withNSAlignment:(NSTextAlignment)alignment onLabel:(id)label andFontSize:(int)fontSize
{
    //  paragraphSetting
